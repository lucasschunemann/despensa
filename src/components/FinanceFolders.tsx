...
import { ChevronRightIcon, CloseIcon, MoreIcon, PlusIcon } from './ControlIcons'
...
export function FinanceFolderNav({ folders, expenses, active, onChange, onAdd, onEdit, selecting, onSelectMode }: {
  ...
  return (
    <aside className="finance-folders" aria-label="Pastas das contas">
      <div className="folder-nav-head"><span>pastas</span><button className="round-control" onClick={onAdd} aria-label="Nova pasta"><PlusIcon size={17} /></button></div>
...
export function FolderEditor({ folder, onClose, onSave, onDelete }: {
  ...
  return (
    <motion.div className="folder-editor" role="dialog" aria-modal="true" aria-labelledby="folder-editor-title" initial={{ opacity: 0, y: 22, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: 0.98 }} transition={{ duration: 0.22, ease: [0.16, 1, 0.3, 1] }}>
      <div className="folder-editor-head"><div><small>{folder ? 'editar pasta' : 'nova pasta'}</small><h3 id="folder-editor-title">um lugar para cada conta</h3></div><button className="round-control" onClick={onClose} aria-label="Fechar"><CloseIcon /></button></div>
      <label className="folder-name-field"><FolderGlyph color={color}/><input ref={input} value={name} onChange={(e) => setName(e.target.value)} placeholder="ex.: casa" maxLength={36} enterKeyHint="done" onKeyDown={(e) => { if (e.key === 'Enter' && name.trim()) onSave(name.trim(), color) }} /></label>
...
export function MoveExpensesTray({ count, folders, onMove, onClose }: {
  ...
  return (
    <motion.div className="move-tray" role="dialog" aria-modal="true" aria-labelledby="move-title" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={reduced ? { duration: 0 } : { duration: 0.32, ease: [0.16, 1, 0.3, 1] }}>
      <div className="move-tray-head"><div><small>{count} {count === 1 ? 'selecionada' : 'selecionadas'}</small><h3 id="move-title">mover para</h3></div><button className="round-control" onClick={onClose} aria-label="Fechar"><CloseIcon /></button></div>
      <div className="move-destinations">
        {folders.map((folder, i) => <motion.button key={folder.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduced ? 0 : i * 0.025 }} onClick={() => onMove(folder.id)}><FolderGlyph color={folder.color}/><span>{folder.name}</span><ChevronRightIcon size={17} /></motion.button>)}
        <button onClick={() => onMove(null)}><FolderGlyph color="slate"/><span>sem pasta</span><ChevronRightIcon size={17} /></button>
      </div>
    </motion.div>
  )
}
...
export function SelectionBar({ count, total, onAll, onMove, onCancel }: { count: number; total: number; onAll: () => void; onMove: () => void; onCancel: () => void }) {
  return (
    <motion.div className="selection-bar" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }}>
      <button className="round-control" onClick={onCancel} aria-label="Cancelar seleção"><CloseIcon size={17} /></button>
